...
if (!empty($tgla)&&!empty($tglb)) {
	$name = "Export-Excel-Pembelian-".$tgla."_s/d_".$tglb;
	$filename = $name;
	
	$sql = "SELECT * FROM tblpembelian_bahan WHERE tanggal BETWEEN '$tgla' AND '$tglb' ORDER BY kode_pembelian ASC";
} else {
	$name = "Export-Excel-Pembelian";
	$filename = $name;
	
	$sql = "SELECT * FROM tblpembelian_bahan ORDER BY kode_pembelian ASC";
}
